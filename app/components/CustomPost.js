...
const CustomPost = ({data}) => {
  ...
  return (
    <View style={{width: '90%', marginHorizontal:'5%', borderRadius: 12, elevation: 4, backgroundColor: '#F0FEFE', padding: 12, borderWidth:1,marginBottom: 10,}}>
        <View style={{flexDirection: 'row', width: '100%', borderWidth: 1, paddingBottom:8, backgroundColor: '#FFFFFF', borderRadius: 12}}>
            <Image source={{uri: data.author.avatarUrl}} style={{marginTop:8, marginLeft: 8, height: 50, width: 50, borderRadius: 25, borderWidth: 1, borderColor: '#354354'}} />
            <View style={{width: '100%', marginLeft: 10, justifyContent: 'center'}}>
                <Text style={{fontSize:20, fontWeight: '700', color: '#354354'}}>{data.author.displayName}</Text>
                <Text style={{fontSize: 14}}>{date}-{month}-{year} {hours}:{mins}</Text>
            </View>
        </View>
        ...
        <View style={{flexDirection: 'row', justifyContent:'space-between', marginTop: 20, marginLeft: 10 }}>
            <View style={{flexDirection:'row', alignItems: 'flex-end'}}>
                <TouchableOpacity activeOpacity={0.6} onPress={()=>{liked===true? unlikePost() : likePost()}}>
                    <AntDesign name={liked===true?'like1':'like2'} color={'#3036D6'} size={25}/>
                </TouchableOpacity>
                <Text style={{marginHorizontal:5, color: '#3036D6'}}>{likes} Likes</Text>
            </View>
            <TouchableOpacity activeOpacity={0.6} onPress={()=>{console.log("Comment Sections")}} style={{flexDirection:'row', alignItems: 'flex-end'}}>
                <AntDesign name='aliwangwang-o1' color={'#3036D6'} size={25} style={{}} />
                <Text style={{marginHorizontal:5, color: '#3036D6'}}>{comments} Comments</Text>
            </TouchableOpacity>
        </View>
    </View>
  )
